...
class Surat extends CI_Controller
{
    ...
    public function updatePesan(){
        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["submitpesan"])) {
            // Pastikan variabel $id sudah didefinisikan sebelumnya
            $pesan = $_POST["pesan"];
            $idd = $_POST["idd"];


            // Lakukan penanganan update di sini
            // Misalnya, update tabel pengajuan_surat dengan menambahkan pesan
            $this->db->set('pesan', $pesan);
            $this->db->where(['id' => $idd]);
            $this->db->update('pengajuan_surat');

            // Set flashdata dan redirect
            $this->session->set_flashdata('success', 'Pesan telah berhasil diperbarui untuk ID: ' . $idd);
