...
class Suratonline extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $this->load->model('Pengajuan_track_model');
    ...
    public function ajukan()
    {
        $status = [
            ...
            3 => 3,  // Sedang Di Proses
            4 => 4,  // Sudah Bisa Di ambil
        ];
        ...
        $this->session->set_flashdata('success', '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h5><i class="icon fas fa-check"></i> Selamat!</h5> Berhasil Mengajukan Surat! Berikut <b>ID</b> anda: <b>' . $id . '</b>Harap Salin Terlebih Dahulu!!</div>');
        redirect(base_url("suratonline"));
    }
    public function updateFile()
    {
        // Pastikan ada file yang dikirimkan
        if (!empty($_FILES['file']['name'])) {
            $file = $_FILES['file'];
    
            // Lakukan validasi ukuran file
            if ($file['size'] >= 5242880) {
                $this->session->set_flashdata('success', '<div class="alert alert-warning alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h5><i class="icon fas fa-ban"></i> MAAF!</h5> File lebih dari 2MB!</div>');
                redirect(base_url("suratonline"));
            }
    
            $namafile = substr($file['name'], -7);
            $jenis_surat = ''; // tentukan jenis surat sesuai kebutuhan Anda
            $file_name = $jenis_surat . uniqid() . $namafile;
            ...
            if ($this->upload->do_upload('file')) {
                ...
                // Mendapatkan ID dari form (asumsi menggunakan input tersembunyi dengan nama 'iddd')
                $id = $this->input->post('iddd');
    
                // Update entri file di database
                $this->Pengajuan_track_model->update_p_file($id, $file_name); // Sesuaikan dengan metode update di model Anda
                $this->session->set_flashdata('success', '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button><h5><i class="icon fas fa-check"></i> Selamat!</h5> Berhasil memperbarui file.</div>');
            } else {
            ...
            // Jika tidak ada file yang diunggah, tampilkan pesan
